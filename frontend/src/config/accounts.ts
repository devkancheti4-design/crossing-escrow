/**
 * The ten well-known Hardhat / Anvil development accounts (mnemonic
 * "test test ... junk"). These keys are PUBLIC and hold nothing on any real
 * network. They exist so a judge can act as every role of the protocol on a
 * local node without switching MetaMask accounts. Demo mode only.
 */
...
export function findDevAccount(address: string): DevAccount | undefined {
  return DEV_ACCOUNTS.find((a) => a.address.toLowerCase() === address.toLowerCase());
}
